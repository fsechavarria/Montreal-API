...
async function GET (req, res) {
	try {
      ...
      id_cel: (req.params.id === null || typeof req.params.id === 'undefined' || isNaN(req.params.id) ) ? undefined : parseInt(req.params.id),
...
async function PUT (req, res) {
  try {
		const id_cel = (req.params.id === null || typeof req.params.id  === 'undefined' || isNaN(req.params.id) ) ? 0 : parseInt(req.params.id)
    if (id_cel != 0) {
      let bindvars = {
				cursor: { type: oracledb.CURSOR, dir : oracledb.BIND_OUT },
        id_cel: id_cel,
