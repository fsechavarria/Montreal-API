...
async function GET (req, res) {
	try {
      ...
      id_curso: (req.params.id === null || typeof req.params.id === 'undefined' || isNaN(req.params.id) ) ? undefined : parseInt(req.params.id),
      ...
      desc_curso: (req.query.desc_curso === null || typeof req.query.desc_curso !== 'string' || req.query.desc_curso.trim().length === 0) ? undefined : req.query.desc_curso.trim(),
      cupos: (req.query.cupos === null || typeof req.query.cupos === 'undefined' || isNaN(req.query.cupos) || String(req.query.cupos).trim().length === 0) ? undefined : parseInt(req.query.cupos),
    }
		let result = []
...
async function POST (req, res) {
  try {
    let bindvars = {
      ...
      desc_curso: (req.body.DESC_CURSO === null || typeof req.body.DESC_CURSO !== 'string' || req.body.DESC_CURSO.trim().length === 0) ? undefined : req.body.DESC_CURSO.trim(),
      cupos: (req.body.CUPOS === null || typeof req.body.CUPOS === 'undefined' || isNaN(req.body.CUPOS) || String(req.body.CUPOS).trim().length === 0) ? undefined : parseInt(req.body.CUPOS),
    }
...
async function PUT (req, res) {
  try {
		const id_curso = (req.params.id === null || typeof req.params.id  === 'undefined' || isNaN(req.params.id ) ) ? 0 : parseInt(req.params.id)
    if (id_curso != 0) {
      let bindvars = {
        cursor: { type: oracledb.CURSOR, dir : oracledb.BIND_OUT },
        id_curso: id_curso,
